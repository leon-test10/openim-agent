...
### OpenHands + DeepSeek single/group E2E

The dedicated Windows smoke test exercises the real OpenIM webhook, OpenHands CLI, DeepSeek API,
and OpenIM reply path for both single and group conversations. It creates a uniquely named test
group, verifies session resume independently for the single chat and group chat, checks group
mention policy and reply-loop protection, then writes a redacted report under `data/validation/`.

Set the API key only in the current process and run the script from the repository root:

```powershell
$env:DEEPSEEK_API_KEY = Read-Host "DeepSeek API key"
.\deploy\windows\smoke-openhands-e2e.ps1
Remove-Item Env:DEEPSEEK_API_KEY
```

The script does not create a `.env` file. It temporarily enables `afterSendGroupMsg` inside the
running OpenIM container and restores the previous webhook file when it exits. Docker Desktop,
healthy `openim-server`/`openim-chat` containers, Node 24, and the OpenHands CLI must be available.
