...
import { RuntimeScopeConfigRepository } from "./core/runtime-scope-config.repository.js";
...
const runners = {
  codex_cli: new CodexCliRunner(codexAdapter),
  template: new TemplateRunner({ responsePrefix: config.TEMPLATE_RUNTIME_RESPONSE_PREFIX }),
  openai_compatible: new OpenAiCompatibleRunner({
  ...
  openhands: new OpenHandsRunner({
    openhandsBin: config.OPENHANDS_BIN,
    openhandsPythonBin: config.OPENHANDS_PYTHON_BIN,
...
const runner = runners[config.RUNTIME_DEFAULT_KIND];
