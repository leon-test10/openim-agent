...
import { RuntimeScopeConfigRepository } from "../../src/core/runtime-scope-config.repository.js";
...
function createTempContext(
  ...
  return {
    ...
    scopeConfigs: new RuntimeScopeConfigRepository(db),
...
describe("Codex vertical slice through AgentRunner", () => {
  it("queues a webhook job, runs the injected runner, records events, and replies to OpenIM", async () => {
    ...
    expect(context.runtimeEvents.listByJobId(jobId).map((event) => event.eventType)).toEqual(
      expect.arrayContaining([
        "bridge.job_queued",
        "bridge.job_running",
        "agent_message",
        "bridge.job_succeeded",
        "bridge.reply_sent"
      ])
    );
  ...
  it("uses the resolved shared runtime for job creation, execution, and status", async () => {
    const sentTexts: string[] = [];
    const codexCalls: string[] = [];
    const openHandsCalls: string[] = [];
    const codexRunner = createRecordingRunner("codex_cli", codexCalls, "CODEX_ACK");
    const openHandsRunner = createRecordingRunner("openhands", openHandsCalls, "OPENHANDS_ACK");
    const context = createTempContext(codexRunner, sentTexts);
    context.runners = {
      codex_cli: codexRunner,
      openhands: openHandsRunner
    };
    context.scopeConfigs.upsert("shared", "__shared__", { runtimeKind: "openhands" });
    const app = await createServer(context, pino({ level: "silent" }));
    ...
    const webhook = await app.inject({
      ...
      payload: {
        ...
        conversationID: "single:codex_bot:bridge_user_1",
        contentType: 101,
        content: JSON.stringify({ content: "use the configured runtime" })
      }
    });
    ...
    expect(openHandsCalls).toHaveLength(1);
    expect(openHandsCalls[0]).toContain(`model=${context.config.OPENAI_COMPATIBLE_MODEL}`);
    expect(context.jobs.getById(jobId)).toMatchObject({
      runtimeKind: "openhands",
      outputText: "OPENHANDS_ACK"
    });
    ...
    expect(runtimeStatus.json()).toMatchObject({
      runtimeKind: "openhands",
      activeSession: { runtimeKind: "openhands" },
      latestJob: { runtimeKind: "openhands" }
    });
  ...
  it("queues a group webhook job when the bot is mentioned or replied to and replies to the group", async () => {
    ...
    const groupRuntimeStatus = await app.inject({
      method: "GET",
      url: "/api/conversations/sg_group_1/runtime-status"
    });
    expect(groupRuntimeStatus.json()).toMatchObject({
      openimConversationId: "group:group_1",
      latestJob: { id: jobId, status: "succeeded" }
    });
...
function createRecordingRunner(
  ...
  return {
    kind,
    run: (input): RuntimeRunHandle => {
      calls.push(`${input.openimConversationId}:${input.inputText}:model=${input.model ?? ""}`);
      return {
        pid: 42,
